...
```bash
bucket-object-archive$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
bucket-object-archive$ sam local start-api
bucket-object-archive$ curl http://localhost:3000/
```
...
```bash
bucket-object-archive$ sam logs -n HelloWorldFunction --stack-name "bucket-object-archive" --tail
```
...
```bash
bucket-object-archive$ pip install pytest pytest-mock --user
